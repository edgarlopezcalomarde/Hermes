...
import com.edloca.hermes.utils.SharedPrefConstants
...
class AuthRepositoryImp @Inject constructor(
    ...
    override fun updateUserInfo(user: UserModel, result: (UiState<String>) -> Unit) {
        db.collection(FireStoreCollections.USERS).document(user.uid.toString())
            .set(user)
            .addOnSuccessListener {
                result.invoke(
                    UiState.Success("User has been update succesfully")
                )
            }
            .addOnFailureListener {
                result.invoke(
        ...
        auth.signInWithEmailAndPassword(email,password)
            .addOnCompleteListener { task ->
                ...
                if (auth.currentUser != null && auth.currentUser!!.isEmailVerified){
                    ...
                    result.invoke(UiState.Failure("Please verify your account first"))
                }
    ...
    override fun storeSession(id: String, result: (UserModel?) -> Unit) {
        db.collection(FireStoreCollections.USERS).document(id)
            .get()
    ...
    override fun getSession(result: (UserModel?) -> Unit) {
      val userPreference = sharedPreferences.getString(SharedPrefConstants.USER_SESSION,null)

        if (userPreference == null){
            result.invoke(null)
        }else{
            val user = gson.fromJson(userPreference,UserModel::class.java)
            result.invoke(user)
        }
